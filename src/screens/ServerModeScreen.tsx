...
export default function ServerModeScreen({ navigation, route }: Props) {
  ...
  const rateBlock = (<>
            ...
            {/* ★★★ DAB CAPTURES AT 2.4 MS/s. 2.048 is TWO OTHER THINGS and this comment used to
                ...
                ★ Only shown where it can matter, and the threshold is 2.048 rather than 2.4 ON
                  PURPOSE: this toggle does not set the rate, it lifts the gate in vsDabCapable(),
                  which asks whether the pinned rate clears kMinCaptureHz (2.048 — the narrowest
                  capture that can still hold a 1.536 MHz ensemble). At 2.048 or above DAB is
                  already offered, so there is nothing to unlock and the control would be a no-op.
                  ★ Worth knowing: DAB RELEASES the pin while it runs (g_vsLockedRate is zeroed on
  ...
  /* ★★ STOPPING THE SERVER IS NOT ONE INTENTION — hence two exits, not one.
   *
   *  There was only "stop & back to servers", and it is the WRONG one for the commonest reason to
   *  stop: you want to change a setting and start again. That was reachable only by stopping,
   *  landing on the picker, re-entering Server mode and scrolling the whole settings page back
   *  down to Start — exactly the walk it took to restart this phone after a test.
   *  ★ Named for where they LAND, not for what they stop: the stop is the same in both and the
   *    destination is the entire difference.
   */
    ...
    return (
      <SafeAreaView style={[styles.root, { backgroundColor: C.bg }]}>
        <ScrollView contentContainerStyle={{ padding: 18, paddingBottom: 40 }}>
          ...
          {/* ★★ TWO WAYS OUT, NOT THREE. The stop is identical in both; the DESTINATION is the
              whole difference, so each is named for where it lands.
              ★ There were briefly three: "back to servers" and "browse the directory" both went
                to the picker — the second merely opened a sheet on arrival — so the screen offered
