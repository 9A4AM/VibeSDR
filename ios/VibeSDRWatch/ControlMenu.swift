...
/// Passband (bandwidth) editor — ported from Jr, thin-remote for Buddy. Two edge boxes (LSB/USB)
/// with a SYNC toggle; TAP a box to select it (it glows), then the crown adjusts that edge. SYNC
/// mirrors both edges around the carrier. Buddy has no demod of its own, so it relays both edges to
/// the phone via WatchLink.setBandwidth (cmd:bw); the phone runs setBandwidth and echoes the result.
struct BandwidthView: View {
  ...
  var body: some View {
    ...
    .onAppear {
      ...
      // Buddy isn't told the demod's symmetric flag; infer it — mirrored edges ⇒ a symmetric mode.
      sync = abs(lo + hi) < 200
      focused = true
    }
  }

  /// Per-detent step: fine for voice, coarse for wide FM. (Jr reads the demod; Buddy infers from width.)
  private func bwStep() -> Double {
    let width = hi - lo
    if width > 40_000 { return 2_000 }
    if width >  8_000 { return 500 }
    return 100
  }
  ...
  private var syncButton: some View {
    Button {
      sync.toggle()
      if sync { let m = max(abs(lo), abs(hi)); lo = -m; hi = m; link.setBandwidth(lo: lo, hi: hi) }
    } label: {
      VStack(spacing: 3) {
