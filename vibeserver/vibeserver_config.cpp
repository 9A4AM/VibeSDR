...
bool fromJson(const std::string& s, Config& c, std::string& err, bool validate) {
    ...
    getBool(s, "rspDabDecim", c.rspDabDecim);
...
void radioFromJson(const std::string& j, RadioConfig& r) {
    ...
    B("dabAgcOverride", r.dabAgcOverride); I("dabAgcTarget", r.dabAgcTarget); B("rfAgc", r.rfAgc); B("rspDabDecim", r.rspDabDecim); I("rfAgcStart", r.rfAgcStart);
...
std::string radioToJson(const RadioConfig& r) {
    ...
    B("autoNotch", r.autoNotch); B("userNotch", r.userNotch);
    B("dabAgcOverride", r.dabAgcOverride); N("dabAgcTarget", r.dabAgcTarget); B("rfAgc", r.rfAgc); B("rspDabDecim", r.rspDabDecim); N("rfAgcStart", r.rfAgcStart);
...
Config effectiveFor(const ServerConfig& s, const RadioConfig& r) {
    ...
    // ★★★ AND THE SAME APPLIES TO THE NAME ON THE RECEIVER SCREEN. Stripping the serial from the
    //     landing page's list was only half of it: this is the name a LISTENER sees painted across
    //     the waterfall once they are connected, and it was still reading "SDRplay RSP1B
    //     240513CA60" (Stuart, 2026-08-09). Same rule, same function, one implementation.
    c.name = publicLabel(r.label.empty() ? s.name : r.label);
